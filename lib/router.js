  ...
  Router.prototype[i] = function(path, ...args){
    const arr = args.map(item => {
      if (typeof item === 'string'){
        ...
        const ctrlSrc = `../api/controllers/${actions.shift()}`
        console.log(ctrlSrc)
        let fn = require(ctrlSrc);
    ...
    console.log(`this.path + path.replace(/\/$/, '') --->`, this.path + path.replace(/\/$/, ''))
    console.log( this.middlewares.concat(arr) )
...
Router.prototype.resources = function(path, ...args){
  ...
  options.only.concat(options.except).reduce((a, b) => a.filter(c => !b.includes(c)))
    .forEach(i => {
      switch (i){
        case 'index':
